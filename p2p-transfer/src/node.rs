...
fn without_trailing_dns_dot(relay: RelayUrl) -> Result<RelayUrl, NodeError> {
    let host = relay
        .host_str()
        .ok_or_else(|| NodeError::Relay("relay has no host".into()))?;
    if !host.ends_with('.') {
        return Ok(relay);
    }
...
pub(crate) fn n0_relays_without_trailing_dots() -> Result<RelayMap, NodeError> {
    // Start with the same map selected by `presets::N0` (including an
    // explicitly forced staging map), changing only the DNS spelling.
    let urls: Vec<RelayUrl> = iroh::endpoint::default_relay_mode().relay_map().urls();
    let urls = urls
        .into_iter()
        .map(without_trailing_dns_dot)
        .collect::<Result<Vec<_>, NodeError>>()?;
    Ok(RelayMap::from_iter(urls))
}

/// Normalize only the relay addresses; the peer ID and every non-relay
/// transport address are retained. A ticket may have been generated by an
/// older sender with a dotted relay URL, even when our own relay is fixed.
pub(crate) fn without_trailing_relay_dots(addr: EndpointAddr) -> Result<EndpointAddr, NodeError> {
    let addrs = addr
        .addrs
        .into_iter()
        .map(|addr| match addr {
            TransportAddr::Relay(url) => Ok(TransportAddr::Relay(without_trailing_dns_dot(url)?)),
            other => Ok(other),
        })
        .collect::<Result<Vec<_>, NodeError>>()?;
