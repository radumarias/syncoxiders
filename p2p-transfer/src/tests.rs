...
use crate::app::{P2PTransfer, ReceivedFile, TorrentInfo};
use crate::node::{ConnectEvent, EchoNode, TransferEvent};
...
fn local_test_chunk_size_calculation() {
    ...
    let file_size_1mb: usize = 1024 * 1024;
    let expected_chunks_1mb = file_size_1mb.div_ceil(CHUNK_SIZE);
    assert_eq!(expected_chunks_1mb, 4, "1MB file should require 4 chunks");

    let file_size_small: usize = 100 * 1024;
    let expected_chunks_small = file_size_small.div_ceil(CHUNK_SIZE);
    assert_eq!(
        expected_chunks_small, 1,
        "100KB file should require 1 chunk"
    );
}

#[test]
fn local_test_empty_file_handling() {
    let test_files: Vec<(String, Vec<u8>)> = vec![("empty.txt".to_string(), Vec::new())];
...
async fn online_test_echo_node_spawn() {
    let result = timeout(Duration::from_secs(3), async { EchoNode::spawn().await }).await;
...
async fn online_test_node_id_persistence() {
    ...
    if let Ok(Ok(node)) = result {
        let node_id_1 = node.endpoint().id();
        let node_id_2 = node.endpoint().id();
...
async fn online_test_small_file_transfer() {
    ...
    if let (Ok(Ok(sender)), Ok(Ok(receiver))) = (sender_result, receiver_result) {
        ...
        while start.elapsed() < Duration::from_secs(5) {
            match timeout(Duration::from_millis(100), stream.next()).await {
                Ok(Some(ConnectEvent::Transfer(TransferEvent::ChunkReceived {
                    chunk_data,
                    ..
                }))) => {
