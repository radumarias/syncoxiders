...
- MultCloud seems to be very similar for Sync and Backup but lacks sync with local files. Also Share is only read only, the files are not kept in sync. Also it’s very buggy, login with Google doesn’t work when you access the shared link., reset pass doesn’t work well, their Google Drive app is still in review and for some reason their Android app is not in store but distributed as APK on their site. Also it lacks encryption
